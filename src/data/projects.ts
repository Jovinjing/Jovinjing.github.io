...
export const projects: Project[] = [
  ...
  },
  {
    ...
    title: 'Aegis Admin',
    tagline: '企业级后台管理系统',
    techStack: ['Vue 3', 'TypeScript', 'Vite', 'Ant Design', 'Zustand', 'React Query', 'NestJS', 'Prisma', 'PostgreSQL', 'Redis', 'JWT', 'i18n'],
    description: `基于 RBAC（Role-Based Access Control）权限模型的企业级后台管理系统，采用前后端分离架构。系统提供完整的权限控制（菜单权限、接口权限、数据权限三级体系）、国际化支持（多语言切换）、日志监控和性能优化方案。`,
    contributions: [
      '搭建前后端分离架构，前端使用 Vue 3 + TypeScript + Vite，后端使用 NestJS + Prisma + PostgreSQL，实现完整的 CRUD 业务模块（用户、角色、菜单、接口管理）',
      '设计并实现 RBAC 三级权限体系（角色 → 权限 → 菜单/接口/数据权限），通过 JWT 双 Token 认证、权限装饰器和拦截器实现细粒度权限控制',
      '集成 Redis 缓存系统，实现菜单树、API 树等高频数据的缓存策略，配合主动失效机制，提升系统响应速度约 60%',
      '实现国际化支持（nestis-i18n），支持自定义语言包和多语言异常信息翻译，通过 Header 动态切换语言',
      '搭建完善的日志系统（Winston + 日志轮转），支持 HTTP/Info/Error 三级日志输出，按天轮转并保留 30 天，方便问题排查',
      '实现请求限流保护（@nestjs/throttler），默认每个 IP 每分钟 100 次请求，防止 API 滥用，保障系统稳定性',
      '前端采用组件化开发策略，封装 100+ 业务组件（按钮、图表、富文本、布局容器等），通过 UnoCSS 实现按需生成 CSS，提升开发效率',
      '使用 ECharts 实现多种图表组件（柱状图、折线图、雷达图、环形图、K 线图、散点图），丰富数据可视化能力',
    ],
    images: [
      '/screenshots/aegis-admin/01-首页.png',
      '/screenshots/aegis-admin/03-用户管理.png',
      '/screenshots/aegis-admin/05-角色管理.png',
      '/screenshots/aegis-admin/07-API管理.png',
      '/screenshots/aegis-admin/09-图表.png',
      '/screenshots/aegis-admin/16-主题切换.png',
      '/screenshots/aegis-admin/21-用户中心.png',
    ],
    demoVideo: '',
    demoUrl: '',
    githubUrl: 'https://github.com/Jovinjing/aegis-admin',
  },
]
